...
export default function CatalogPanel({ catalogState, agentColor }) {
  ...
  useEffect(() => {
    ...
    if (action === 'show_home') {
      ...
      transition(() => {
        setView({ type: 'comparison' })
        setCompare(catalogState.products || [])
      })
    } else if (action === 'show_promotion') {
      setPromotion({
        title: catalogState.title,
        description: catalogState.description,
        discount_pct: catalogState.discount_pct,
      })
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {CATEGORIES.map(cat => (
          <button
            key={cat.id}
            onClick={() => fetchAndShowCategory(cat.id)}
  ...
  const renderCategory = () => {
    ...
    // Derive subcategory filter tabs from available products
    const subcats = ['All', ...new Set(products.map(p => p.subcategory).filter(Boolean))]
    const showSubcatFilter = subcats.length > 2

    // Filter by active subcategory
    const filtered = activeSubcat && activeSubcat !== 'All'
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        {showSubcatFilter && (
          <div className="flex gap-2 mb-4 flex-wrap">
            {subcats.map(sc => {
              ...
              const isActive = activeSubcat === sc || (sc === 'All' && activeSubcat === 'All')
              return (
                <button
                  key={sc}
        ...
        {/* Product grid — 2 columns so each card has more room */}
        <div className="grid grid-cols-2 gap-4 flex-1">
          {pageProducts.map(p => (
            <ProductCard
              key={p.sku}
              product={p}
              highlighted={p.sku === highlightedSku}
              onSelect={p => setView({ type: 'detail', product: p })}
            />
          ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-3 mt-4 pt-3 border-t border-gray-100">
            <button
              onClick={() => setView(v => ({ ...v, page: Math.max(1, v.page - 1) }))}
              disabled={safePage === 1}
              className="px-3 py-1.5 text-sm rounded-lg border disabled:opacity-40 hover:bg-gray-50 transition"
            >
              ← Prev
            </button>
            <span className="text-sm text-gray-500">Page {safePage} of {totalPages}</span>
            <button
              onClick={() => setView(v => ({ ...v, page: Math.min(totalPages, v.page + 1) }))}
  ...
  return (
    <div className="h-full flex flex-col bg-cream overflow-hidden relative">
      ...
      {promotion && (
        <div
          className="absolute top-0 left-0 right-0 z-20 p-4 text-white text-center shadow-lg"
      ...
      {/* Main content */}
      <div
        className="flex-1 overflow-y-auto p-5 transition-all duration-350"
      ...
      >
        ...
        {view.type === 'detail'     && (
          <ProductDetail product={view.product} onBack={() => setView({ type: 'home' })} />
        )}
